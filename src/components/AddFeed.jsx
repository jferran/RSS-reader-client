...
function AddFeed() {
  ...
  return (
    <div>
      ...
      {foundFeeds!=null && foundFeeds.map((element, index)=> <div>{index}: <img style={{height: '16px'}} src={`https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${element.url}&size=128`} alt=''/> {element.title} {element.url}<button onClick={handleSubscribe} value={element.url} title={element.title}>Subscribe</button></div>)}
